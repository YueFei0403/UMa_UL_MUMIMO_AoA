...
sigPower = rms(sigGridMultiDU(:)).^2;
noiseGridMultiDU = sigPower/sqrt(2*10^(5/10)).*complex(randn(size(sigGridMultiDU)),randn(size(sigGridMultiDU)));
end
...
x_sqval = trace(x*x');
yExtracted = y*x';
W_MMSE = Beta / (Beta*x_sqval + trace(n*n'));
